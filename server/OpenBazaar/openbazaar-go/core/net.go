...
func (n *OpenBazaarNode) SendOrder(peerId string, contract *pb.RicardianContract) (err error) {
	p, err := peer.IDB58Decode(peerId)
	if err != nil {
		return err
	}
...
func (n *OpenBazaarNode) SendCancel(peerId string,contract *pb.RicardianContract) error {
	a, err := ptypes.MarshalAny(contract)
	if err != nil {
		return err
	}
	...
	//	kp = &k
	//}
	k, err := libp2p.UnmarshalPublicKey(contract.GetVendorListings()[0].GetVendorID().GetPubkeys().Identity)
	if err != nil {
		return err
	}
...
func (n *OpenBazaarNode) SendReject(peerId string,k *libp2p.PubKey, rejectMessage *pb.RicardianContract) error {
	p, err := peer.IDB58Decode(peerId)
